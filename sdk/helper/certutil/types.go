...
func (p *ParsedCertBundle) GetKeyBits() (int, error) {
	...
	case x509.MLDSA:
		...
		case mldsa.MLDSA87():
			return 87, nil
		default:
			return -1, fmt.Errorf("unknown parameter set for ML-DSA Public Key: %v", pub.Parameters())
		}
	default:
...
func (c *CSRBundle) ToParsedCSRBundle() (*ParsedCSRBundle, error) {
	...
	if len(c.PrivateKey) > 0 {
		...
		default:
			...
			} else if t, err := getPKCS8Type(pemBlock.Bytes); err == nil {
...
func (p *ParsedCSRBundle) getSigner() (crypto.Signer, error) {
	...
	case Ed25519PrivateKey:
		...
		var ok bool
		signer, ok = signerd.(ed25519.PrivateKey)
		if !ok {
			return nil, errutil.UserError{Err: fmt.Sprintf("Unable to parse CA's private Ed25519 key: got %T", signerd)}
		}
	...
	case MLDSAPrivateKey:
		...
		var ok bool
		signer, ok = signerd.(*mldsa.PrivateKey)
		if !ok {
			return nil, errutil.UserError{Err: fmt.Sprintf("Unable to parse CA's private ML-DSA key: got %T", signerd)}
		}

	default:
		return nil, errutil.UserError{Err: "Unable to determine type of private key; only RSA, Ed25519, ML-DSA and EC are supported"}
	}
	return signer, nil
}
